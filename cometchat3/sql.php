...
$sql_queries['admin_message_count'] = "select (select count(id) from cometchat where sent >= '{sent}') +  (select count(id) from cometchat_archive where sent >= '{sent}') + (select count(id) from cometchat_chatroommessages where sent >= '{sent}') + (select count(id) from cometchat_chatroommessages_archive where sent >= '{sent}') as totalmessages";
...
$sql_queries['getuserchatings'] = "select count(*) as users from (select DISTINCT cometchat.from userid from cometchat where cometchat.sent > '{sent}' UNION select DISTINCT cometchat_archive.from userid from cometchat_archive where cometchat_archive.sent > '{sent}' UNION SELECT DISTINCT cometchat_chatroommessages.userid userid FROM cometchat_chatroommessages WHERE cometchat_chatroommessages.sent > '{sent}' UNION SELECT DISTINCT cometchat_chatroommessages_archive.userid userid FROM cometchat_chatroommessages_archive WHERE cometchat_chatroommessages_archive.sent > '{sent}') x";
...
$sql_queries['admin_viewuser'] = "select distinct(f.{usertable_userid}) id, f.{usertable_username} username from cometchat m1, {usertable} f where (f.{usertable_userid} = m1.from and m1.to = '{userid}') or (f.{usertable_userid} = m1.to and m1.from = '{userid}') UNION select distinct(f.{usertable_userid}) id, f.{usertable_username} username from cometchat_archive m2, {usertable} f where (f.{usertable_userid} = m2.from and m2.to = '{userid}') or (f.{usertable_userid} = m2.to and m2.from = '{userid}') {guestpart} order by username";
...
$sql_queries['cron_groupusers'] = "delete from cometchat_chatrooms_users where cometchat_chatrooms_users.userid in (select cometchat_chatrooms_users.userid from cometchat_status where lastactivity < ({lastactivity}-3600))
...
$sql_queries['clearPrivateMessage'] = "delete from cometchat where cometchat.sent<{time}-10800";
...
*/
$sql_queries['archive_privatemessage'] = "INSERT INTO cometchat_archive SELECT * from cometchat where cometchat.sent<{time}-10800";
$sql_queries['archive_guestentries'] = "INSERT INTO cometchat_guests_archive SELECT * from cometchat_guests where id in (select userid from cometchat_status where cometchat_status.lastactivity<{time}-10800 && userid >{firstguestid})";

$sql_queries['archive_gueststatus'] = "INSERT INTO cometchat_status_archive SELECT * from cometchat_status where userid in (select userid from cometchat_status where cometchat_status.lastactivity<{time}-10800 && userid >{firstguestid})";
...
class SqlQueries{
	function getQueries(){
		...
		$sqlqueries['auth_getFriendsList'] = "select DISTINCT ".DB_USERTABLE.".".DB_USERTABLE_USERID." userid, ".DB_USERTABLE.".".DB_USERTABLE_NAME." username, ".DB_USERTABLE.".".DB_LINKFIELD." link, ".DB_AVATARFIELD." avatar, cometchat_status.lastactivity lastactivity, cometchat_status.lastseen lastseen, cometchat_status.lastseensetting lastseensetting, cometchat_status.status, cometchat_status.message, cometchat_status.isdevice, cometchat_status.readreceiptsetting readreceiptsetting from ".DB_USERTABLE." left join cometchat_status on ".DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid ".DB_AVATARTABLE." where {timestampCondition} (cometchat_status.status IS NULL OR cometchat_status.status <> 'invisible' OR cometchat_status.status <> 'offline') order by username asc";

		$sqlqueries['getOfflineUsers'] = "select ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." userid, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." username, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." link, ".DB_AVATARFIELD." avatar, cometchat_status.lastactivity lastactivity, cometchat_status.lastseen lastseen, cometchat_status.lastseensetting lastseensetting, cometchat_status.status, cometchat_status.message, cometchat_status.isdevice, cometchat_status.readreceiptsetting readreceiptsetting from  ".TABLE_PREFIX.DB_USERTABLE."   left join cometchat_status on ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid ".DB_AVATARTABLE." where (cometchat_status.status IS NULL OR cometchat_status.status = 'offline' OR cometchat_status.lastactivity < {time} - ".((ONLINE_TIMEOUT)*2).") order by username asc limit {limit}";
		...
		$sqlqueries['auth_getActivechatboxdetails'] = "select DISTINCT ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." userid, ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_NAME." username,  ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." link, ".DB_AVATARFIELD." avatar, cometchat_status.lastactivity lastactivity, cometchat_status.lastseen lastseen, cometchat_status.lastseensetting lastseensetting, cometchat_status.status, cometchat_status.message, cometchat_status.isdevice, cometchat_status.readreceiptsetting readreceiptsetting from ".TABLE_PREFIX.DB_USERTABLE." left join cometchat_status on ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." = cometchat_status.userid ".DB_AVATARTABLE." where ".TABLE_PREFIX.DB_USERTABLE.".".DB_USERTABLE_USERID." IN ({userids})";
	...
	*/
		...
		$sqlqueries['get_guest_users'] = "select count(cometchat_guests.id) as totalguestusers from cometchat_guests";

		$sqlqueries['admin_getActiveUsersCount'] = "SELECT COUNT(userid) AS usercount, IF(userid > '{firstguestid}', 'guest', 'user') AS usertype FROM ( SELECT cometchat_chatroommessages.userid AS userid FROM cometchat_chatroommessages WHERE sent >= '{sent}' GROUP BY userid UNION SELECT cometchat_chatroommessages_archive.userid AS userid FROM cometchat_chatroommessages_archive WHERE sent >= '{sent}' GROUP BY userid UNION SELECT cometchat.from AS userid FROM cometchat WHERE sent >= '{sent}' GROUP BY userid UNION SELECT cometchat_archive.from AS userid FROM cometchat_archive WHERE sent >= '{sent}' GROUP BY userid ) AS activeusers GROUP BY usertype";

		$sqlqueries['admin_getlast24hrsPrivateMessageCount'] = "select count(id) as messagecount from cometchat where sent < ".time()." - 10800";

		$sqlqueries['admin_getlast24hoursGroupMessageCount'] = "select count(id) as messagecount from cometchat_chatroommessages where sent < ".time()." - 10800";
	...
	*/
	...
	/*START: Graph Queries*/
$sqlqueries['today_messages'] = "SELECT SUM(privatemessage) AS privatemessage, SUM(groupmessage) AS groupmessage FROM ( SELECT COUNT(id) AS privatemessage, 0 AS groupmessage FROM `cometchat` AS m WHERE m.sent > UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) UNION SELECT COUNT(id) AS privatemessage, 0 AS groupmessage FROM `cometchat_archive` AS am WHERE am.sent > UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) UNION SELECT 0 AS privatemessage, COUNT(id) AS groupmessage FROM `cometchat_chatroommessages` AS gm WHERE gm.sent > UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR) UNION SELECT 0 AS privatemessage, COUNT(id) AS groupmessage FROM `cometchat_chatroommessages_archive` AS gam WHERE gam.sent > UNIX_TIMESTAMP(NOW() - INTERVAL 24 HOUR)) AS X";

$sqlqueries['weekly_privateMessages'] = "SELECT COUNT(id) AS message, DAYNAME(FROM_UNIXTIME(sent)) AS day from (SELECT id, cometchat.sent FROM cometchat UNION SELECT id, cometchat_archive.sent FROM cometchat_archive ) as temp WHERE DATE(FROM_UNIXTIME(sent)) >= DATE(NOW()) - INTERVAL 6 DAY GROUP BY DAY";

$sqlqueries['weekly_groupMessages'] = "SELECT COUNT(id) AS message, DAYNAME(FROM_UNIXTIME(sent)) AS day from (SELECT id, cometchat_chatroommessages.sent FROM cometchat_chatroommessages UNION SELECT id, cometchat_chatroommessages_archive.sent FROM cometchat_chatroommessages_archive ) as temp WHERE DATE(FROM_UNIXTIME(sent)) >= DATE(NOW()) - INTERVAL 6 DAY GROUP BY DAY";

$sqlqueries['monthly_Messages'] = "SELECT SUM(privatemessage) AS privatemessage, SUM(groupmessage) AS groupmessage, WEEK AS WEEK FROM ( SELECT * FROM ( SELECT COUNT(id) AS privatemessage, 0 AS groupmessage, WEEK(FROM_UNIXTIME(sent)) AS WEEK FROM ( SELECT id, cometchat.sent FROM cometchat UNION SELECT id, cometchat_archive.sent FROM cometchat_archive ) AS temp WHERE DATE(FROM_UNIXTIME(sent)) >= DATE(NOW()) - INTERVAL 30 DAY GROUP BY WEEK) AS privatetbl UNION SELECT * FROM ( SELECT 0 AS privatemessage, COUNT(id) AS groupmessage, WEEK(FROM_UNIXTIME(sent)) AS WEEK FROM ( SELECT id, cometchat_chatroommessages.sent FROM cometchat_chatroommessages UNION SELECT id, cometchat_chatroommessages_archive.sent FROM cometchat_chatroommessages_archive ) AS temp WHERE DATE(FROM_UNIXTIME(sent)) >= DATE(NOW()) - INTERVAL 30 DAY GROUP BY WEEK) AS grouptbl ) AS monthmessages GROUP BY WEEK ORDER BY WEEK ASC";

$sqlqueries['all_Messages'] = "SELECT SUM(privatemessage) AS privatemessage, SUM(groupmessage) AS groupmessage, SUBSTR(MONTHNAME, 1, 3) AS month, SUBSTR(YEAR, 3, 2)  as year FROM ( SELECT * FROM ( SELECT COUNT(id) AS privatemessage, 0 AS groupmessage, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME FROM ( SELECT id, cometchat.sent FROM cometchat UNION SELECT id, cometchat_archive.sent FROM cometchat_archive ) AS temp GROUP BY MONTHNAME) AS privatetbl UNION SELECT * FROM ( SELECT 0 AS privatemessage, COUNT(id) AS groupmessage, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME FROM ( SELECT id, cometchat_chatroommessages.sent FROM cometchat_chatroommessages UNION SELECT id, cometchat_chatroommessages_archive.sent FROM cometchat_chatroommessages_archive ) AS temp GROUP BY MONTHNAME) AS grouptbl ) AS monthmessages GROUP BY MONTHNAME, YEAR ORDER BY YEAR ASC";

$sqlqueries['weekly_active_users'] = "SELECT COUNT(userid) as usercount, IF(userid > {firstguestid}, 'guest','user') as usertype, date as day from (SELECT * FROM ( SELECT userid AS userid, DAYNAME(date) AS date FROM ( SELECT cometchat_chatroommessages.userid AS userid, DATE(FROM_UNIXTIME(cometchat_chatroommessages.sent)) as date FROM cometchat_chatroommessages GROUP BY userid, date UNION SELECT cometchat_chatroommessages_archive.userid AS userid, DATE(FROM_UNIXTIME(cometchat_chatroommessages_archive.sent)) as date FROM cometchat_chatroommessages_archive GROUP BY userid, date ) AS temp WHERE date >= DATE(NOW()) - INTERVAL 6 DAY) AS groups UNION SELECT * FROM (SELECT userid, DAYNAME(date) AS date FROM ( SELECT cometchat.from AS userid, DATE(FROM_UNIXTIME(cometchat.sent)) as date FROM cometchat GROUP BY userid, date UNION SELECT cometchat_archive.from AS userid, DATE(FROM_UNIXTIME(cometchat_archive.sent)) as date FROM cometchat_archive GROUP BY userid, date ) AS temp WHERE date >= DATE(NOW()) - INTERVAL 6 DAY) AS oneoneone) as activeusers group by date, usertype";

$sqlqueries['monthly_active_users'] = "SELECT COUNT(userid) AS usercount, IF(userid > {firstguestid}, 'guest', 'user') AS usertype, WEEK as week, YEAR AS year FROM ( SELECT * FROM ( SELECT userid AS userid, WEEK(DATE) AS WEEK, YEAR(DATE) AS YEAR FROM ( SELECT cometchat_chatroommessages.userid AS userid, DATE( FROM_UNIXTIME( cometchat_chatroommessages.sent ) ) AS DATE FROM cometchat_chatroommessages GROUP BY userid, DATE UNION SELECT cometchat_chatroommessages_archive.userid AS userid, DATE( FROM_UNIXTIME( cometchat_chatroommessages_archive.sent ) ) AS DATE FROM cometchat_chatroommessages_archive GROUP BY userid, DATE ) AS temp WHERE DATE >= DATE(NOW()) - INTERVAL 30 DAY) AS groups UNION SELECT * FROM ( SELECT userid, WEEK(DATE) AS WEEK, YEAR(DATE) AS YEAR FROM ( SELECT cometchat.from AS userid, DATE(FROM_UNIXTIME(cometchat.sent)) AS DATE FROM cometchat GROUP BY userid, DATE UNION SELECT cometchat_archive.from AS userid, DATE( FROM_UNIXTIME(cometchat_archive.sent) ) AS DATE FROM cometchat_archive GROUP BY userid, DATE ) AS temp WHERE DATE >= DATE(NOW()) - INTERVAL 30 DAY) AS oneoneone ) AS activeusers GROUP BY WEEK, usertype ORDER BY year, week";

$sqlqueries['all_active_users'] = "SELECT COUNT(userid) AS usercount, IF(userid > {firstguestid}, 'guest', 'user') AS usertype, SUBSTR(MONTHNAME, 1, 3) AS month, MONTH AS MONTHNO, SUBSTR(YEAR, 3, 2) AS year FROM ( SELECT * FROM ( SELECT userid AS userid, MONTHNAME, MONTH, YEAR FROM ( SELECT cometchat_chatroommessages.userid AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat_chatroommessages GROUP BY userid, MONTHNAME UNION SELECT cometchat_chatroommessages_archive.userid AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat_chatroommessages_archive GROUP BY userid, MONTHNAME ) AS temp ) AS groups UNION SELECT * FROM ( SELECT userid, MONTHNAME AS MONTHNAME, MONTH, YEAR FROM ( SELECT cometchat.from AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat GROUP BY userid, MONTHNAME UNION SELECT cometchat_archive.from AS userid, YEAR(FROM_UNIXTIME(sent)) AS YEAR, MONTHNAME(FROM_UNIXTIME(sent)) AS MONTHNAME, MONTH(FROM_UNIXTIME(sent)) AS MONTH FROM cometchat_archive GROUP BY userid, MONTHNAME ) AS temp ) AS oneoneone ) AS activeusers GROUP BY MONTHNAME, YEAR, usertype ORDER BY year,MONTHNO";

	$sqlqueries['all_groups'] = "SELECT COUNT(id) AS groupcount, monthno, SUBSTR(MONTH, 1, 3) as month, SUBSTR(YEAR, 3, 2) AS year FROM ( SELECT cometchat_chatrooms.id, MONTH( FROM_UNIXTIME(cometchat_chatrooms.createdon) ) AS monthno, MONTHNAME( FROM_UNIXTIME(cometchat_chatrooms.createdon) ) AS MONTH, YEAR( FROM_UNIXTIME(cometchat_chatrooms.createdon) ) AS YEAR FROM `cometchat_chatrooms` UNION SELECT cometchat_chatrooms_archive.id, MONTH( FROM_UNIXTIME(cometchat_chatrooms_archive.createdon) ) AS monthno, MONTHNAME( FROM_UNIXTIME(cometchat_chatrooms_archive.createdon) ) AS MONTH, YEAR( FROM_UNIXTIME( cometchat_chatrooms_archive.createdon ) ) AS YEAR FROM `cometchat_chatrooms_archive` ) AS X GROUP BY month,year ORDER BY year,monthno";
