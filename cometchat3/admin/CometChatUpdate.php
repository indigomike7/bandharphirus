...
*/
	...
	public $livesoftware;
	public $querystring;
	function __construct($latest_v,$integration){
		...
		try {
			if(empty($token)){
				...
				if (is_array($data) &&  array_key_exists("error",$data) && $data['error'] == 1) {
					$response['error'] = 1;
					$response['message'] = 'Unable to make request to download CometChat api, please download the CometChat and manually place it in /cometchat/writable/updates/'.$this->$latest_v.'/cometchat.zip ';
				} else {
					file_put_contents($file, $data);
					if (filesize($file) > 0) {
						$response = array('error'=>0,'message'=> 'CometChat zip downloaded succssfully');
					} else {
						$response = array('error'=>1,'message'=> 'Unable to save the zip file please download the CometChat and manually place it in /cometchat/writable/updates/'.$this->latest_v.'/cometchat.zip');
					}
				}
			}
		} catch (Exception $e) {
...
*/
	public function createHash() {
		try {
			...
			$exclude = array('.','..','writable','CHANGELOG.txt','integration.php','cloud','cometchat_update.php','environment.default.php','environment.php','localhost.default.php','localhost.php',"api/api.php");
			@session_start();
	...
	public function generateHash() {
		try {
			...
			foreach($files as $key => $value){
				...
				if(!is_dir($path) ) {
					if($value != 'integration.php' && $value !='cloud' && $value !='cometchat_update.php' && $value != 'environment.default.php' && $value != 'environment.php' && $value != 'localhost.default.php' && $value != 'localhost.php'){
						$hash = md5_file($path);
						$filehash[] = $hash;
...
global $update, $latest_vesion;
$update = new CometChatUpdate($latest_vesion,$cms);
