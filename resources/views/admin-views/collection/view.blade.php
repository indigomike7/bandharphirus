...
<div class="content container-fluid">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page">{{trans('messages.collection')}}</li>
        </ol>
    </nav>
    <!-- Page Heading -->
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    {{ trans('messages.collection_form')}}
                </div>
                <div class="card-body">
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $('#id').val(data.id);
                    $('#name').val(data.name);
                    $('#Imageviewer').attr('src', "{{asset('storage/app/public/collection')}}/"+data.icon);
                    $('#cate-table').hide();
                    $('#add').html("{{ trans('messages.update')}}");
                    $("form").attr('action', "{{route('admin.collection.update')}}");
                    fetch_category()
                }
            });
        });
