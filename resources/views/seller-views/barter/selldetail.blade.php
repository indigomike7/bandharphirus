...
//									window.location.href = "/seller/premium/success/?payment_method=tripay&amount="+ amount + "&days=" +days;
							...
							success: function (data) {
								if (data.errors) {
									...
									setInterval(function () {
										location.href = '{{route("seller.barter.orderlistsell")}}';
									}, 2000);
								}
							}
						});
                ...
                <form class="product-form" action="{{route('seller.barter.updateorderdeliverystatusseller')}}" method="post" enctype="multipart/form-data"
                      id="product_form">
                            <div class="form-group">
...
@push('script')
    ...
    <script>
        $('#product_form').submit(function (e) {
            ...
            $.post({
                url: '{{route('seller.barter.updateorderdeliverystatusseller')}}',
                data: formData,
                contentType: false,
                processData: false,
                success: function (data) {
                    if (data.errors) {
                        ...
                        setInterval(function () {
                            location.href = '{{route("seller.barter.selldetail",[$sbo->id])}}';
                        }, 2000);
                    }
                }
            });
