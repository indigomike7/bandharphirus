...
@section('title','Update To Premium')
...
@endif                        <div class="card-body row">
							...
							30 Days<br/>
							Cost : {{$ps->cost_30_days}}<br/>
                                <input type="hidden" name="cost_30_days" value="{{$ps->cost_30_days}}" class="form-control" id="cost_30_days">
								<button type="button" onclick="$('#payment_30').show();$('#payment_90').hide();$('#payment_180').hide();$('#payment_365').hide();">Apply</button>
                            </div>

                            <div class="form-group col-md-2" style="border:1px solid black; margin:30px;">
							90 Days<br/>
							Cost : {{$ps->cost_90_days}}<br/>
                                <input type="hidden" name="cost_90_days" value="{{$ps->cost_90_days}}" class="form-control" id="cost_90_days" >
								<button type="button"onclick="$('#payment_30').hide();$('#payment_90').show();$('#payment_180').hide();$('#payment_365').hide();">Apply</button>
                            </div>
                            <div class="form-group col-md-2" style="border:1px solid black;  margin:30px;">
							180 Days<br/>
							Cost : {{$ps->cost_180_days}}<br/>
                                <input type="hidden" name="cost_180_days" value="{{$ps->cost_180_days}}" class="form-control" id="cost_180_days" >
								<button type="button"onclick="$('#payment_30').hide();$('#payment_90').hide();$('#payment_180').show();$('#payment_365').hide();">Apply</button>
                            </div>
                            <div class="form-group col-md-2" style="border:1px solid black;  margin:30px;">
							365 Days<br/>
							Cost : {{$ps->cost_365_days}}<br/>
								<input type="hidden" name="cost_365_days" value="{{$ps->cost_365_days}}" class="form-control" id="cost_365_days" placeholder="100000">
									...
									<div class="card">
										<div class="card-body" style="height: 100px">
											<a class="btn btn-block"
											   href="javascript:modalview('{{$data['data'][$i]['code']}}',$('#cost_30_days').val(),30);">
							...
							@for($i=0;$i<count($data['data']);$i++)
								<div class="col-md-6 mb-4" style="cursor: pointer">
									<div class="card">
										<div class="card-body" style="height: 100px">
											<a class="btn btn-block"
											   href="javascript:modalview('{{$data['data'][$i]['code']}}',$('#cost_90_days').val(),90);">
												{{$data['data'][$i]['name']}} <br/>Code : {{$data['data'][$i]['code']}}
											</a>
										</div>
									</div>
							...
							@for($i=0;$i<count($data['data']);$i++)
								<div class="col-md-6 mb-4" style="cursor: pointer">
									<div class="card">
										<div class="card-body" style="height: 100px">
											<a class="btn btn-block"
											   href="javascript:modalview('{{$data['data'][$i]['code']}}',$('#cost_180_days').val(),180);">
												{{$data['data'][$i]['name']}} <br/>Code : {{$data['data'][$i]['code']}}
											</a>
										</div>
									</div>
							...
							@for($i=0;$i<count($data['data']);$i++)
								<div class="col-md-6 mb-4" style="cursor: pointer">
									<div class="card">
										<div class="card-body" style="height: 100px">
											<a class="btn btn-block"
											   href="javascript:modalview('{{$data['data'][$i]['code']}}',$('#cost_365_days').val(),365);">
												{{$data['data'][$i]['name']}} <br/>Code : {{$data['data'][$i]['code']}}
											</a>
										</div>
									</div>
